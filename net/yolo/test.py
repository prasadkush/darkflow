...
def postprocess(self, net_out, im, ground_truth, save = True):
	...
	Takes net output, draw predictions, save to disk
	"""
	# ground_truth contains list of boxes for the image. each element in list has the following: [class_name, class_idx, xmin, ymin, xmax, ymax]
	...
	for i in range(len(predictboxes)):
		...
		for j in range(len(ground_truth)):
			if predictboxes[i][0] == ground_truth[j][0]:
				iou = compute_iou(predictboxes[i], ground_truth[j])
				if iou > 0.4:
					true_positive += 1
		if true_positive > 0:
			true_positives += 1
	...
	for i in range(len(ground_truth)):
		...
		for j in range(len(predictboxes)):
			if ground_truth[i][0] == predictboxes[j][0]:
				iou = compute_iou(predictboxes[j], ground_truth[i])
				if iou > 0.4:
					true_positive = 1
		if true_positive == 0:
			false_negatives += 1
...
def box_union(a, b):
	...
	union_ab = area_a + area_b - box_intersection(a,b)
	return union_ab

def compute_iou(a, b):
	iou = box_intersection(a,b)/box_union(a,b)
	return iou
